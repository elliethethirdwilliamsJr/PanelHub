...
def _rewrite_m3u8(text: str, base_url: str, proxy_base: str, referer: Optional[str]):
    ...
    for raw in lines:
        ...
        if line.startswith('#') and 'URI="' in line:
            def _sub(m, _b=base_url, _pb=proxy_base, _r=referer):
                ...
                return f'URI="{q}"'
            out.append(_re.sub(r'URI="([^"]+)"', _sub, line))
            continue
...
@app.options('/proxy/hls')
async def proxy_hls_options() -> Response:
...
@app.get('/proxy/hls')
async def proxy_hls(request: Request, url: str = Query(...), ref: Optional[str] = Query(None)) -> Response:
    try:
        target_url = _up.unquote(url)
    ...
    headers = {
        ...
        'Accept': '*/*',
        'Accept-Language': 'en-US,en;q=0.9',
        'Referer': ref or HEADERS.get('Referer'),
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=20.0) as client:
        ...
        # If upstream returns an error (404/403 etc), retry with minimal headers
        if resp.status_code not in (200, 206):
            try:
                minimal_headers = {'User-Agent': HEADERS.get('User-Agent')}
                resp2 = await client.get(target_url, headers=minimal_headers)
            except Exception:
                resp2 = None
            ...
            if resp2 and resp2.status_code in (200, 206):
                ...
                # Try with referer set to the target host root (some CDNs require referer)
                try:
                    referer_hint = f"{parsed.scheme}://{parsed.netloc}/"
                    referer_headers = {'User-Agent': HEADERS.get('User-Agent'), 'Referer': referer_hint}
                    resp3 = await client.get(target_url, headers=referer_headers)
                except Exception:
                    resp3 = None

                if resp3 and resp3.status_code in (200, 206):
                    resp = resp3
                else:
                    # All retries failed — return a diagnostic response with CORS headers
                    cors_headers = {
                        'Access-Control-Allow-Origin': '*',
                        'Access-Control-Allow-Methods': 'GET, OPTIONS',
                        'Access-Control-Allow-Headers': 'Range, Content-Type',
                    }
                    primary_status = getattr(resp, 'status_code', None)
                    minimal_status = getattr(resp2, 'status_code', None) if 'resp2' in locals() else None
                    referer_status = getattr(resp3, 'status_code', None) if 'resp3' in locals() else None
                    primary_headers = dict(resp.headers) if resp is not None and hasattr(resp, 'headers') else {}
                    snippet = ''
                    try:
                        snippet = resp.text[:1000] if resp is not None and hasattr(resp, 'text') else ''
                    except Exception:
                        snippet = ''

                    debug = (
                        f"Upstream fetch failed for {target_url}\n"
                        f"primary_status: {primary_status}\n"
                        f"minimal_retry_status: {minimal_status}\n"
                        f"referer_retry_status: {referer_status}\n\n"
                        f"-- primary_headers --\n{json.dumps(primary_headers, indent=2)}\n\n"
                        f"-- body_snippet (first 1000 chars) --\n{snippet}\n"
                    )
                    return Response(content=debug, media_type='text/plain', status_code=502, headers=cors_headers)
    ...
    response_headers = {**cors_headers, 'Content-Type': content_type}
    if resp.headers.get('Content-Length'):
        response_headers['Content-Length'] = resp.headers['Content-Length']
    if resp.headers.get('Content-Range'):
        response_headers['Content-Range'] = resp.headers['Content-Range']

    is_m3u8 = ('mpegurl' in content_type.lower() or target_url.split('?')[0].endswith('.m3u8'))
    if is_m3u8:
        proxy_base = f"{request.url.scheme}://{request.url.netloc}/proxy/hls"
        rewritten = _rewrite_m3u8(resp.text, target_url, proxy_base, ref)
        return Response(content=rewritten, media_type='application/vnd.apple.mpegurl', headers=response_headers)
