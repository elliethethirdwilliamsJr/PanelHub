...
function proxyM3u8(url, referer) {
    ...
    // Prefer explicit PROXY_BASE, then fallback to global __MIRURO_API_URL__ if present.
    let proxyBase = (window.PROXY_BASE || PROXY_BASE || '').toString().trim();
    if (!proxyBase && window.__MIRURO_API_URL__) {
        try {
            const api = (window.__MIRURO_API_URL__ || '').toString().trim().replace(/\/+$/,'');
            if (api) proxyBase = `${api}/proxy/hls`;
        } catch (_) { proxyBase = ''; }
    }
...
async function playHLS(rawUrl, allStreams = null) {
    ...
    // Probe proxied manifest; if proxy returns 404, try raw URL as a fallback
    try {
        if (proxiedUrl && proxiedUrl !== rawUrl) {
            const probe = await fetch(proxiedUrl, { method: 'HEAD' });
            if (probe.status === 404) {
                try {
                    const probeRaw = await fetch(rawUrl, { method: 'HEAD' });
                    if (probeRaw.ok) finalUrl = rawUrl;
                } catch (_) {
                    // fall through, keep proxiedUrl
                }
            }
        }
    } catch (_) {
        // probe failed — continue with proxiedUrl
    }
